...
class ServicesTests {
    ...
    private lateinit var services: services.Services

    @Test
    fun testCreateInvite() {
        runBlocking {
            val res = services.createInvite(
            ...
            assertNotNull(res)
            assertEquals("INaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.sid)
            assertEquals("ACaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.accountSid)
            assertEquals("CHaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.channelSid)
            assertEquals("ISaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.serviceSid)
            assertEquals("identity", res.identity)
            assertEquals("RLaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.roleSid)
        }
    }

    @Test
    fun testCreateMemberInChannel() {
        runBlocking {
            ...
            assertNotNull(res)
            assertEquals("MBaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.sid)
            assertEquals("ACaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.accountSid)
            assertEquals("CHaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.channelSid)
            assertEquals("ISaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.serviceSid)
            assertEquals("identity", res.identity)
    ...
    fun testCreateMessage() {
        runBlocking {
            ...
            assertNotNull(res)
            assertEquals("IMaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.sid)
            assertEquals("ACaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.accountSid)
            assertEquals("CHaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.channelSid)
            assertEquals("ISaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", res.serviceSid)
            assertEquals("Hello", res.body)
            assertEquals("system", res.from)
        }
    }

    @BeforeAll
    fun submitOpenApiSpecAndScenarios() {
        runBlocking {
            ...
            val memberScenario = Scenario(
                name = "Create Member",
                path = "/v1/services.Services/{ServiceSid}/Channels/{ChannelSid}/Members",
                method = "POST",
                responses = listOf(
                    ResponseConfig(
                        statusCode = "201",
            ...
            val messageScenario = Scenario(
                name = "Create Invite",
                path = "/v1/services.Services/{ServiceSid}/Channels/{ChannelSid}/Messages",
                method = "POST",
                responses = listOf(
                    ResponseConfig(
                        statusCode = "201",
            ...
            try {
                ...
                client.post("http://localhost:8081/admin/response") {
                    contentType(ContentType.Application.Json)
                    setBody(inviteScenario)
                    header("Transaction-token", token)
                }.body<TransactionToken>().transactionToken

                client.post("http://localhost:8081/admin/response") {
                    contentType(ContentType.Application.Json)
                    setBody(memberScenario)
                    header("Transaction-token", token)
                }.body<TransactionToken>().transactionToken

                client.post("http://localhost:8081/admin/response") {
                    contentType(ContentType.Application.Json)
                    setBody(messageScenario)
                    header("Transaction-token", token)
                }.body<TransactionToken>().transactionToken
                ...
                val twilio = services.TwilioMock(client2, host)

                services = services.Services(twilio)
